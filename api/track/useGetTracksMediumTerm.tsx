...
export function useGetTracksMediumTerm() {
    const [dataTracksMediumTerm, setDataTracksMediumTerm] = useState<TrackType | null>(null);
    ...
    useEffect(() => {
        ...
        const fetchTrackData = async () => {
            try {
                ...
                const response = await axios.get("https://api.spotify.com/v1/me/top/tracks?time_range=medium_term&limit=20&offset=0", headers);
                const res = response.data;

                setDataTracksMediumTerm(res);
                setLoading(false);
            } catch (error) {
    ...
    return { dataTracksMediumTerm, loadingMedium, error };
}
